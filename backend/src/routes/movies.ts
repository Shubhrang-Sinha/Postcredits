...
export function movieRoutes(app: Hono) {
  // List movies
  ...
  app.post("/movies", authMiddleware, async (c) => {
    ...
    const { title, directorId, duration, releaseYear, genreIds } =
      await c.req.json();
  ...
  app.put("/movies/:movieId", authMiddleware, async (c) => {
    ...
    const movie = await queryOne(
      "SELECT work_id FROM movies WHERE movie_id = ?",
      [movieId],
    );
    if (!movie) {
      return c.json({ error: "Movie not found" }, 404);
    ...
    await execute(
      "UPDATE works SET title = ?, release_year = ? WHERE work_id = ?",
      [title, releaseYear, movie.work_id],
    );
    await execute(
      "UPDATE movies SET director_id = ?, duration = ? WHERE movie_id = ?",
      [directorId, duration, movieId],
    );

    if (genreIds) {
      await execute("DELETE FROM work_genres WHERE work_id = ?", [
        movie.work_id,
      ]);
      for (const gid of genreIds) {
        await execute(
