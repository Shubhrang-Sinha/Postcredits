...
export function bookRoutes(app: Hono) {
  // List books
  ...
  app.post("/books", authMiddleware, async (c) => {
    ...
    const { title, authorId, pages, releaseYear, genreIds } =
      await c.req.json();
  ...
  app.put("/books/:bookId", authMiddleware, async (c) => {
    ...
    const book = await queryOne("SELECT work_id FROM books WHERE book_id = ?", [
      bookId,
    ]);
    if (!book) {
      return c.json({ error: "Book not found" }, 404);
    ...
    await execute(
      ...
      [title, releaseYear, book.work_id],
    );
    await execute(
      "UPDATE books SET author_id = ?, pages = ? WHERE book_id = ?",
      [authorId, pages, bookId],
    );
