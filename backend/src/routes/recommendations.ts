...
export function recommendationRoutes(app: Hono) {
  ...
  app.get('/recommendations/blend', authMiddleware, async (c) => {
    ...
    const recommendations = await generateBlendRecommendations(user.userId, type, limit);
    
    return c.json(recommendations);
  });
}

async function calculateSimilarity(user1Id: number, user2Id: number): Promise<number> {
  ...
  const similarity = Math.round(100 * (1 - (diff[0]?.total_diff || 0) / maxDiff));
  
  return Math.max(0, similarity);
}

async function generateBlendRecommendations(userId: number, workType: string, limit: number) {
  ...
  const typeCondition = workType !== 'all' ? `AND w.work_type = '${workType}'` : '';
