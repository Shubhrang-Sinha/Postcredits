...
export async function seed() {
  ...
  try {
    ...
    const books: BookRow[] = csvParse(booksCsv, {
      columns: true,
    ...
    const moviesCsv = fs.readFileSync(
      path.join(dataDir, "movies.csv"),
      "utf-8",
    );
    const movies: MovieRow[] = csvParse(moviesCsv, {
      columns: true,
      skip_empty_lines: true,
    });
    ...
    const creators = new Map<
      string,
      { name: string; type: "author" | "director" }
    >();
    ...
    const [genreRows] = await connection.execute<mysql.RowDataPacket[]>(
    ...
    for (const [key, value] of creators) {
      ...
      try {
        await connection.execute(
          "INSERT IGNORE INTO creators (name, creator_type) VALUES (?, ?)",
          [name, type],
        );
      } catch (e) {
        // Ignore duplicate errors
    ...
    const [creatorRows] = await connection.execute<mysql.RowDataPacket[]>(
      "SELECT creator_id, name, creator_type FROM creators",
    );
    const creatorMap = new Map(
      creatorRows.map((r) => [`${r.name}|${r.creator_type}`, r.creator_id]),
    );
