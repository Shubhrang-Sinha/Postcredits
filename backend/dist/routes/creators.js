...
export function creatorRoutes(app) {
    // List all creators
    ...
    app.get("/creators/:creatorId", async (c) => {
        ...
        const creator = await queryOne("SELECT creator_id, name, creator_type, created_at FROM creators WHERE creator_id = ?", [creatorId]);
        if (!creator) {
    ...
    app.post("/creators", authMiddleware, async (c) => {
        ...
        const result = await execute("INSERT INTO creators (name, creator_type) VALUES (?, ?)", [name, creatorType]);
    ...
    app.put("/creators/:creatorId", authMiddleware, async (c) => {
        ...
        await execute("UPDATE creators SET name = COALESCE(?, name), creator_type = COALESCE(?, creator_type) WHERE creator_id = ?", [name || null, creatorType || null, creatorId]);
