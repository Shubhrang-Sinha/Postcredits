...
export function ratingRoutes(app) {
    ...
    app.post('/ratings', authMiddleware, async (c) => {
        ...
        const work = await queryOne('SELECT work_id, work_type FROM works WHERE work_id = ?', [workId]);
        if (!work) {
            return c.json({ error: 'Work not found' }, 404);
        }
        // Check for existing rating (upsert)
        const existing = await queryOne('SELECT rating_id FROM ratings WHERE user_id = ? AND work_id = ?', [user.userId, workId]);
        let ratingId;
        if (existing) {
            await execute('UPDATE ratings SET score = ?, updated_at = NOW() WHERE rating_id = ?', [score, existing.rating_id]);
            ratingId = existing.rating_id;
        }
        else {
            const result = await execute('INSERT INTO ratings (user_id, work_id, score) VALUES (?, ?, ?)', [user.userId, workId, score]);
            ratingId = result.insertId;
        }
        const ratedWork = await queryOne('SELECT title, work_type FROM works WHERE work_id = ?', [workId]);
        return c.json({ ratingId, workId, score, workType: ratedWork?.work_type, title: ratedWork?.title });
    });
    // Update rating
    app.put('/ratings/:ratingId', authMiddleware, async (c) => {
        ...
        // Verify ownership
        const rating = await queryOne('SELECT user_id FROM ratings WHERE rating_id = ?', [ratingId]);
        if (!rating) {
        ...
        await execute('UPDATE ratings SET score = ?, updated_at = NOW() WHERE rating_id = ?', [score, ratingId]);
