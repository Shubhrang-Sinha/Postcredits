...
async function seed() {
    ...
    try {
        ...
        const books = csvParse(booksCsv, { columns: true, skip_empty_lines: true });
        console.log(`  Loaded ${books.length} books`);
        console.log('Loading movies.csv...');
        const moviesCsv = fs.readFileSync(path.join(dataDir, 'movies.csv'), 'utf-8');
        const movies = csvParse(moviesCsv, { columns: true, skip_empty_lines: true });
        ...
        const [genreRows] = await connection.execute('SELECT genre_id, name FROM genres');
        ...
        const [creatorRows] = await connection.execute('SELECT creator_id, name, creator_type FROM creators');
        ...
        for (const book of books) {
            ...
            if (!authorId || !genreId)
                continue;
            try {
                const [workResult] = await connection.execute('INSERT INTO works (title, work_type, release_year) VALUES (?, ?, ?)', [book.title, 'book', parseInt(book.release_year) || 2000]);
                const workId = workResult.insertId;
                await connection.execute('INSERT INTO books (work_id, author_id, pages) VALUES (?, ?, ?)', [workId, authorId, parseInt(book.pages) || 200]);
                await connection.execute('INSERT INTO work_genres (work_id, genre_id) VALUES (?, ?)', [workId, genreId]);
                booksInserted++;
            }
            catch (e) {
        ...
        for (const movie of movies) {
            ...
            if (!directorId || !genreId)
                continue;
            try {
                const [workResult] = await connection.execute('INSERT INTO works (title, work_type, release_year) VALUES (?, ?, ?)', [movie.title, 'movie', parseInt(movie.release_year) || 2020]);
                const workId = workResult.insertId;
                await connection.execute('INSERT INTO movies (work_id, director_id, duration) VALUES (?, ?, ?)', [workId, directorId, parseInt(movie.duration) || 90]);
                await connection.execute('INSERT INTO work_genres (work_id, genre_id) VALUES (?, ?)', [workId, genreId]);
                moviesInserted++;
            }
            catch (e) {
